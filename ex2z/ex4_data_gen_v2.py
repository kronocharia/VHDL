        ...
        for ((xs,ys),(xin,yin),b) in point_list: # loop over all lines to draw, starting at (0,0)
            ...
            while (x,y) != (xn,yn): # loop outputting data for one line
                ...
                if  (errx > errdiag) or ((errx == errdiag) and (b == 0)):
                    ...
                    x = x + 1
                    error = error + incry
                (x,y,b) = doswaps(swaps,(x,y,b), inverse=1)
...
def doswaps((swapxy,negx,negy), (x,y,b), inverse=0):
    if (not inverse) and swapxy:
        (x,y) = (y,x)
        b = 1-b
    if negx:
        x = -x
...
draw('ex4_data_pak3.vhd',[ ((0,0),(1,2),0), ((2,3),(0,0),0), ((9,4),(5,3),1)])

draw('ex4_data_pak2.vhd',[ ((0,0),(5,8),0), ((2046,112),(2051,104),1)])
